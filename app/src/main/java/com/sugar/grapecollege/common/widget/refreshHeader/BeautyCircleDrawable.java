...
public class BeautyCircleDrawable extends Drawable {
    ...
    public BeautyCircleDrawable() {
        mPaint = new Paint();
        mPaint.setColor(QsHelper.getInstance().getApplication().getResources().getColor(R.color.color_accent));
        mPaint.setFlags(Paint.ANTI_ALIAS_FLAG);
        mPaint.setStyle(Paint.Style.FILL);
        list = new ArrayList<>();
        int pointRadius = CommonUtils.dp2px(2.0f);
        for (int i = 0; i < 8; i++) {
            CirclePoint circlePoint = new CirclePoint(pointRadius, i, 8);
...
//        canvas.restoreToCount(count);
    ...
    public void setIsReachCriticalPoint(boolean isReachPoint) {
